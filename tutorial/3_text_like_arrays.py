...
class FormulaColor1(Scene): 
    def construct(self):
        text = TexMobject("x","=","{a","\\over","b}").scale(4)
#        text = TexMobject("x","=","\\frac{a}","{b}").scale(4) NO Funciona: no se puede cortar el \frac
        text[0].set_color(RED)
        text[1].set_color(BLUE)
        text[2].set_color(GREEN)
        text[3].set_color(ORANGE)
...
class CheckFormula3(CheckFormulaByTXT):
    CONFIG={
#        "text": TexMobject("\\sqrt{","\\int_{","a}^","{b}","\\left(","\\frac{x}{y}","\\right)","dx}")
        "text": TexMobject(r"\sqrt{\int_{a}^{b} \left(\frac{x}{y}\right)dx")
    }
