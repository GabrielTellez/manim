...
class TransformVSReplacementTransform(Scene):
	def construct(self):
			...
			self.play(ReplacementTransform(M1.copy(),M2))
			self.wait()

			self.play(Transform(M2,M1))
			self.wait()
...
class CopyTextV1new(Scene):
	def construct(self):
		...
		for pre,post in changes:
			# print(pre,post)
			# print([(i,j) for i,j in zip(pre,post)])
			self.play(
				*[
...
class ChangeColorAndSizeAnimation2(Scene):
	def construct(self):
		...
		text.target.shift(2*UP+RIGHT)
		text.target.scale(3)
		text.target.set_color(BLUE)
