...
#A few simple shapes
    def construct(self):
        ...
        triangle=Polygon(np.array([0,0,1]),np.array([1,1,0]),np.array([1,-1,0]))
...
#        plane.main_lines.fade(.9)
    ...
    def calc_field(self,point):
        ...
        result = Vector(efield).shift(point) #Create vector and shift it to grid point
        return result
...
#        plane.main_lines.fade(.9)
        ...
        field = VGroup(*[self.calc_field(x*RIGHT+y*UP+z*IN)
            for x in np.arange(-9,9,0.5)
            for y in np.arange(-5,5,0.5)
